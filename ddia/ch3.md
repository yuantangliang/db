...
数据库需要具有两个最基本的核心功能：当你给它一些数据时，它应该其存储起来，当你查询这些数据时，它应该将其返回给你。

在第二章中，我们讨论了数据模型和查询语言，即你（应用开发者）如何向数据库存储数据，以及你如何通过数据库查询数据。在本章中，我们从数据库的角度再次讨论这个问题：我们如何存储用户提供的数据，以及当用户读取时，我们如何查找数据。

您作为应用程序开发者，为什么要关心数据库是如何存储和检索数据的呢？您可能不会从头开始编写存储引擎，但是世界上可用的存储引擎这么多，你需要从中选择和你的工作负载最匹配的。只有你对数据库运行的原理有一个大概的了解，你才能根据工作负载的要求，优化存储引擎。

特别需要注意的是，存储引擎可以针对事务性负载进行优化，也可以针对分析负载优化，它们之间存在着巨大差异。稍后我们将在第90页的 “事务处理或分析？” 和第91页的 “列存储”中探讨这个区别，那里将讨论一系列针对分析优化的存储引擎。

然而，在本章中，我们将首先介绍通用的数据引擎，它们被各大数据库采用，其中包括你可能很熟悉的：传统关系型数据库与所谓的“NoSQL”数据库。我们会介绍两大类存储引擎：日志结构（log-structured）的存储引擎和面向页面（page-oriented）的存储引擎（如B树）。
...
这两个函数实现了具有键值存储功能的数据库。你可以通过执行`db_set key value`，将`key`和`value`存储在数据库中。键和值可以是（几乎）任何你喜欢的东西，例如，值可以是JSON文档。你可以通过调用`db_get key`，查找与该键关联的最新值并将其返回。麻雀虽小，五脏俱全：
